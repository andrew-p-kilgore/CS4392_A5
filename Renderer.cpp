...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	gui_1->addVariable("X1", m_bone_animation->x1);
	gui_1->addVariable("Y1", m_bone_animation->y1);
	gui_1->addVariable("Z1", m_bone_animation->z1);
	gui_1->addVariable("X2", m_bone_animation->x2);
	gui_1->addVariable("Y2", m_bone_animation->y2);
	gui_1->addVariable("Z2", m_bone_animation->z2);
	gui_1->addVariable("X3", m_bone_animation->x3);
	gui_1->addVariable("Y3", m_bone_animation->y3);
	gui_1->addVariable("Z3", m_bone_animation->z3);
...
void Renderer::draw_bones(Shader& shader, Bone_Animation* m_bone_animation)
{
	...
	Object* bone_obj2 = nullptr;
	Object* bone_obj3 = nullptr;
	Object* bone_obj4 = nullptr;
	for (unsigned int i = 0; i < obj_list.size(); i++)
	{
		if (obj_list[i].obj_name == "cube") {
			bone_obj = &obj_list[i];
			bone_obj2 = &obj_list[i];
			bone_obj3 = &obj_list[i];
			bone_obj4 = &obj_list[i];
		}
	}
	if (bone_obj == nullptr)
		return;
	...
	// Draw bone 2
	glm::mat4 bone2 = m_bone_animation->root->children[0]->matrix;
	bone2 = glm::scale(bone2, m_bone_animation->scale_vector[1]);
	glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(bone2));
	bone_obj2->obj_color = m_bone_animation->colors[1];
	draw_object(shader, *bone_obj2);

	// Draw bone 3
	glm::mat4 bone3 = m_bone_animation->root->children[1]->matrix;
	bone3 = glm::scale(bone3, m_bone_animation->scale_vector[2]);
	glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(bone3));
	bone_obj3->obj_color = m_bone_animation->colors[2];
	draw_object(shader, *bone_obj3);

	// Draw bone 4
	glm::mat4 bone4 = m_bone_animation->root->children[2]->matrix;
	bone4 = glm::scale(bone4, m_bone_animation->scale_vector[3]);
	glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(bone4));
	bone_obj4->obj_color = m_bone_animation->colors[3];
	draw_object(shader, *bone_obj4);
